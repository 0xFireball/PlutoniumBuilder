...
namespace EtoDesignerHost
{
    ...
    public class EtoDesignerLoader : DesignerLoader
    {
        ...
        public override void BeginLoad(IDesignerLoaderHost host)
        {
            ...
            if (fileName == null)
            {
                var etoPlatform = Eto.Platform.Instance; //Force loading Eto
        ...
        private void UpdateCodeWindows()
        {
            ...
            TextBox xmlWindow = tc.TabPages[2].Controls[0] as TextBox;
                        ...
                        case 1:
                            {
                                ...
                                CSharpCodeProvider cs = new CSharpCodeProvider();
                                //cs.CreateGenerator().GenerateCodeFromCompileUnit(codeCompileUnit, sw, o);
                                sw.WriteLine(GenerateEtoCodeFromDom(cs, codeCompileUnit));
                                sw.Close();
                            }
                            break;
                        case 2:
                            {
                                ...
                                sw.WriteLine(GenerateEtoCodeFromDom(vb, codeCompileUnit));
                                sw.Close();
                            }
                            break;
                        case 3:
        ...
        internal bool Build()
        {
            ...
            if (executable != null)
            {
                ...
                //string sourceCode = GenerateCodeFromDom(cc, codeCompileUnit);
                string sourceCode = GenerateEtoCodeFromDom(cc, codeCompileUnit);

                //CompilerResults cr = cc.CompileAssemblyFromDom(cp, codeCompileUnit);
                CompilerResults cr = cc.CompileAssemblyFromSource(cp, sourceCode);
